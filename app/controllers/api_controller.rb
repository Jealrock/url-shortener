class ApiController < ApplicationController
    ...
    def truncate
        params[:desired] = UrlPair.generate_random_shortened_path if params[:shortened].blank?
        @url_pair = UrlPair.new(original: params[:original], shortened: params[:shortened])
        if @url_pair.save
    ...
    private
        ...
        def set_user_token
            UrlPair.generate_user_token unless params[:user_token]
        end
            ...
            @url_pairs.map do |item|
                {
                    ...
                    'shortened': item[1],
                    'visits_count': item[2],
                    'expiring': (item[3] - 14.days).beginning_of_day.to_i
                }
            end
        end
end
